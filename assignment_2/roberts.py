...
threshold = 50 
_, thresholded_image = cv2.threshold(filtered_image, threshold, 255, cv2.THRESH_BINARY)

# cv2.imshow("Original Image", image)
# cv2.imshow("Roberts Edge Detection", filtered_image)
# cv2.imshow("Roberts Edge Detection (Thresholded)", thresholded_image)
cv2.imshow("Original vs. Roberts Edge Detection vs. Roberts Edge Detection (Thresholded)", np.hstack((image, filtered_image, thresholded_image)))
cv2.waitKey(0)
